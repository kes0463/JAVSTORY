...
| **Stage 5** | **Build** | 🚧 진행 중 | `build_master_db.py` 구현 (GUI 라이브러리 연동 예정). |
| **Stage 6** | **Maintain** | 🔄 대기 | 에러 복구(04_ERROR) 폴더 관리 및 데이터 마이그레이션. |
...
- [ ] **라이브러리 (Library)**: 통합 검색 필터 및 씬 탐색 (예정).
- [ ] **세팅 (Settings)**: API 키 및 공장 루트 설정 (예정).
...
> [!IMPORTANT]
> **Stage 5: Build (Consolidation) 및 라이브러리 뷰 통합**
> - 각 영상의 개별 분석 결과(`web_database.json`)를 `master_db.js`로 통합하고, 라이브러리 화면에서 이를 브라우징할 수 있도록 구현할 차례입니다.
