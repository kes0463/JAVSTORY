#!/usr/bin/env python3
"""
onejav.com/torrent/{slug} 직접 GET (하이픈 없는 소문자 슬러그).
프로젝트 루트: python scripts/test_onejav_crawl.py SNOS-152
"""
...
def main() -> int:
    p = argparse.ArgumentParser(description="onejav 토렌트 페이지 크롤 테스트")
    add_standard_args(p)
    args = p.parse_args()
    ...
    url = f"https://onejav.com/torrent/{slug}"
    host = (urlparse(url).hostname or "").lower()

    proxy = resolve_proxy(getattr(args, "proxy", None))
    print_fetch_banner("onejav", url)
