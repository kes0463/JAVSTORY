...
### scene_summary 작성 지침 (매우 중요)
- 단순 사건 나열이 아니라 **그 장면을 실제로 보는 듯한** 현장감, **야한 긴장감·애태움·숨 막히는 흥분**을 살린다.
- 신음, 호흡, 손·입·다리의 움직임, 상대의 굴욕/항복 반응 등을 **구체적으로** 쓴다.
- “매우 에로틱하다”, “흥분된다”, “야하다” 같은 **메타 평가 문장**은 넣지 말고, **상황 묘사**로 관객이 느끼게 한다.
- AV 아카이브·번역 품질용이므로 **불필요한 자기검열·뉴스체 순화**를 하지 말 것.

### time_range 작성 지침
- **웹 검색 우선:** 품번·タイトル로 다음을 **검색해 시도**한다 — 판매/公式の「チャプター」「収録時間」「収録内容」「プレイ内容」「章」、レビュー・まとめサイトの**開始時刻・尺・パート別のざっくり時間**、総再生時間(本編尺)。Grokのウェブ検索で**大まかなタイムラインやパート区切り**は多くの作品でヒットするので、**いきなり均等割りせず**、ヒットした時刻・尺を優先して `time_range` に反映する。
- **다중 사이트 교차 검증 (필수):** 한 사이트·한 리뷰의 타임코드만 믿지 말 것. **서로 다른 출처를 최소 2~3곳 이상** 조회해 비교한다 (예: 공식/판매 페이지·DMM/FANZA 계열 메타, Javlibrary·javdb·일본어 요약·챕터 후기, 해외 아카이브형 DB 등). 출처마다 시각이 다르면 **전체 본편 길이(총 재생 시간)** 안에서 모순을 조정하고, **공식·카탈로그·판매 페이지에 가까운 수치**를 우선하되, 여전히 어긋나면 **겹치는 구간·중앙에 가까운 범위**로 맞춘 뒤 `(추정)`을 붙인다. 단일 블로그의 극단값만 따르지 말 것.
- **교차 검증 시 조회할 참고 도메인 (품번 검색에 섞어 사용; 전부 필수는 아님):** `https://www.dmm.co.jp` (공식·수록 메타 우선), `https://javtiful.com/`, `https://javfas.com/`, `https://javarchive.com/`, `https://www.njav.com/`, `https://jav.guru/`, `https://javmost.to/`, `https://javtrailers.com/`, `https://javquick.com/`, `https://javmobile.net/`, `https://missav.live/` — 위와 **기존에 언급한 Javlibrary·javdb 등**을 함께 검색해 타임라인·챕터 단서를 맞춘다. 스트리밍·집계 사이트는 **메타/시간 정보 보조**로만 쓰고, 제목·품번·출연 일치는 반드시 교차 확인한다.
- 検索で**一部パートだけ**時刻が分かれば、そのパートはそれに合わせ、不明なパートだけ残り時間を按分する。
- 確認できれば `HH:MM:SS ~ HH:MM:SS`（出典が明確なら `(推定)` なしでもよい）。
- 検索でも区間の手がかりがなく、**総尺とシーン数だけ**しか無い場合のみ均等割り等で `00:12:34 ~ 00:18:50 (추정)` とする — 全体尺の範囲内で論理的に。 **`XX` プレースホルダ禁止**（常に実数の `HH:MM:SS`）。 **`(시간 미상)` は出さない。**
