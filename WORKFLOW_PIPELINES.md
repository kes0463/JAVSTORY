...
├── build_master_db.py           # 웹용 master_db.js 빌드
...
│                    라이브러리 + Export                                │
│                                                                     │
│   canonical 스키마 → Grok 병합 → 스틸 추출 → master_db.js 빌드      │
└─────────────────────────────────────────────────────────────────────┘
```
...
| `build_master_db.py` | 웹용 DB 빌드 | Factory 산출물 → `data/derived/master_db.js` |
...
| `library/export/` | 번들 생성, 지문(fingerprint), `master_db.js` 빌드, 동기화 drift |
...
```
build_master_db.py
  └─ javstory.library.export.master_js.write_master_db_js()
       └─ Factory/03_COMPLETED/**/web_database.json 수집
       └─ data/derived/master_db.js 생성
       └─ web/index.html에서 로드
```
